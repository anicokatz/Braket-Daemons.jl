...
function parse_ahs_program(ahs_json, args::Dict{String, Any}, file_name::String)
    ...
    # generate path, save everything in hdf5 format
    path = joinpath(experiment_path, "parsed_ahs_info.h5")
    h5open(path, "w") do fw
        write(fw, "filling", Vector{Float64}(filling)) # cast to float for saving
        write(fw, "atom_coordinates", array_2d_coords)
    end
...
function compute_MPS_evolution(ψ::MPS, circuit::Vector{Vector{ITensor}}, max_bond_dim::Int, cutoff::Float64, τ::Float64, Deltas::Vector{Float64}, Omegas::Vector{Float64};
    ...
    # always instantiate optional calculations during runtime
    err_array = zeros(ComplexF64, n_τ_steps)
    meas_array = Array{ComplexF64}(undef, N, n_τ_steps)
    zz_corr_array = Array{ComplexF64}(undef, N, N, n_τ_steps)
    fid_susc_array = zeros(ComplexF64, n_τ_steps)
    ...
    for i_τ in 1:n_τ_steps
        ...
        if compute_observables
            ...
            SzSz = real(correlation_matrix(ψ, "Sz", "Sz"));
            zz_corr_array[:, :, i_τ] = 4 .* SzSz
...
function run(ahs_json, args, file_name::String="ahs_object")
    ...
    compute_observables = args["compute-observables"]
    compute_fidelity_susceptibility = args["compute-fidelity-susceptibility"]
    ...
    if compute_observables
        results["meas_array"] = meas_array
        results["zz_corr_array"] = zz_corr_array
    end

    if compute_truncation_error
        results["err_array"] = err_array
    end

    if compute_fidelity_susceptibility
        results["fid_susc_array"] = fid_susc_array
    end
...
function save_results(results, processed_results_path::String)

    # generate path, save everything in hdf5 format
    path = joinpath(processed_results_path, "processed_results.h5")

    h5open(path, "w") do fw

        # final shots; always generated
        write(fw, "final_shots", results["samples"])

        # optional outputs
        if haskey(results, "meas_array")
            write(fw, "density", results["meas_array"])
        end

        if haskey(results, "zz_corr_array")
            write(fw, "correlator_zz", results["zz_corr_array"])
        end

        if haskey(results, "err_array")
            write(fw, "truncation_error", results["err_array"])
        end

        if haskey(results, "fid_susc_array")
            write(fw, "fidelity_susceptibility", results["fid_susc_array"])
        end
